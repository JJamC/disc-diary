...
const seed = async ({ commentsData, postsData, usersData, albumsData, tracksData }) => {
...
async function insertAlbums(albumsData) {
  ...
  const insertAlbumQueryStr = format(
    ...
    albumData.map(({title, author_id, body, cover_art, album_id}) => [
       title, author_id, body, cover_art, album_id
    ])
  );
...
async function insertPosts(postsData) {
     const insertPostsQueryStr = format(
       ...
       tracksData.map(({ author_id, body, album_id, votes = 0, created_at }) => [
         title,
         body,
         cover_art,
         album,
